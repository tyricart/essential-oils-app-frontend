...
const AdminOil = () => {
  ...
  function sendOil(newOil) {
    const { oilName, medicinalUse, emotionalUse, meansOfUse, chakra, safety } =
      newOil;
  ...
  const handleDelete = (event) => {
    ...
    fetch(`http://localhost:4040/oils/remove-oil?oilName=${oilName}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div className="container-oil">
      ...
      <form className="add-form">
        ...
        <button onClick={() => handleDelete(oil)}>Delete Oil</button>
      </form>
    </div>
  );
};
